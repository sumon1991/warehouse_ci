...
class Reports extends MY_Controller
{
    ...
    function index()
    {
        $this->sma->checkPermissions(13,33);
    ...
    function warehouse_stock($warehouse = NULL)
    {
		...
		$this->sma->checkPermissions(13,34,true);
        $data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    function expiry_alerts($warehouse_id = NULL)
    {
       // $this->sma->checkPermissions('expiry_alerts');
		$this->sma->checkPermissions(13,63);
    ...
    function quantity_alerts($warehouse_id = NULL)
    {
       // $this->sma->checkPermissions('quantity_alerts');
		$this->sma->checkPermissions(13,36);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    function getQuantityAlerts($warehouse_id = NULL, $pdf = NULL, $xls = NULL)
    {
		...
		$this->sma->checkPermissions(13,36,true);
        if (!$this->Owner && !$warehouse_id) {
            $user = $this->site->getUser();
            $warehouse_id = $user->warehouse_id;
        }
    ...
    {
		$this->sma->checkPermissions(13,0);
    ...
    function products()
    {
		...
		$this->sma->checkPermissions(13,37);
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
...
$this->sma->checkPermissions(13,37,true);
    ...
    function categories()
    {
		...
		$this->sma->checkPermissions(13,38);
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['categories'] = $this->site->getAllCategories();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        if ($this->input->post('start_date')) {
    ...
    {
       // $this->sma->checkPermissions('products', TRUE);
		$this->sma->checkPermissions(13,38,true);
        $warehouse = $this->input->get('warehouse') ? $this->input->get('warehouse') : NULL;
        $category = $this->input->get('category') ? $this->input->get('category') : NULL;
        $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
        $end_date = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;
    ...
    {
      //  $this->sma->checkPermissions('products');
		$this->sma->checkPermissions(13,39);
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['brands'] = $this->site->getAllBrands();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        if ($this->input->post('start_date')) {
    ...
    {
        //$this->sma->checkPermissions('products', TRUE);
		$this->sma->checkPermissions(13,39,true);
        $warehouse = $this->input->get('warehouse') ? $this->input->get('warehouse') : NULL;
        $brand = $this->input->get('brand') ? $this->input->get('brand') : NULL;
        $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
        $end_date = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;
    ...
    {
        //$this->sma->checkPermissions();
		$this->sma->checkPermissions(13,40);
    ...
    {
        //$this->sma->checkPermissions();
		$this->sma->checkPermissions(13,41);
        if (!$this->Owner && !$this->Admin && $this->session->userdata('warehouse_id')) {
            $warehouse_id = $this->session->userdata('warehouse_id');
        }
        if (!$year) {
    ...
    function sales()
    {
       // $this->sma->checkPermissions('sales');
		$this->sma->checkPermissions(13,42);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
       // $this->sma->checkPermissions('sales', TRUE);
		$this->sma->checkPermissions(13,42,true);
        ...
        if ($pdf || $xls) {
            ...
            $si = "( SELECT sale_id, product_id, serial_no, GROUP_CONCAT(CONCAT({$this->db->dbprefix('sale_items')}.product_name, '__', {$this->db->dbprefix('sale_items')}.quantity) SEPARATOR '___') as item_nane from {$this->db->dbprefix('sale_items')} GROUP BY {$this->db->dbprefix('sale_items')}.sale_id ) FSI";
            $this->load->library('datatables');
            $this->datatables
    ...
    function getQuotesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $qi = "( SELECT quote_id, product_id, GROUP_CONCAT(CONCAT({$this->db->dbprefix('quote_items')}.product_name, '__', {$this->db->dbprefix('quote_items')}.quantity) SEPARATOR '___') as item_nane from {$this->db->dbprefix('quote_items')} GROUP BY {$this->db->dbprefix('quote_items')}.quote_id ) FQI";
            $this->load->library('datatables');
            $this->datatables
    ...
    function purchases()
    {
		...
		$this->sma->checkPermissions(13,47);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
...
$this->sma->checkPermissions(13,47,true);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        ...
        if ($pdf || $xls) {
            ...
            $pi = "( SELECT purchase_id, product_id, (GROUP_CONCAT(CONCAT({$this->db->dbprefix('purchase_items')}.product_name, '__', {$this->db->dbprefix('purchase_items')}.quantity) SEPARATOR '___')) as item_nane from {$this->db->dbprefix('purchase_items')} GROUP BY {$this->db->dbprefix('purchase_items')}.purchase_id ) FPI";
    ...
    function payments()
    {
		...
		$this->sma->checkPermissions(13,43);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
...
$this->sma->checkPermissions(13,43,true);
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : NULL;
    ...
    function customers()
    {
       // $this->sma->checkPermissions('customers');
		$this->sma->checkPermissions(13,49);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
...
$this->sma->checkPermissions(13,49,true);
        if ($pdf || $xls) {
    ...
    function suppliers()
    {
       // $this->sma->checkPermissions('suppliers');
		$this->sma->checkPermissions(13,50);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
...
$this->sma->checkPermissions(13,50,true);
        if ($pdf || $xls) {
    ...
    function users()
    {
		$this->sma->checkPermissions(13,51);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    function getUsers()
    {$this->sma->checkPermissions(13,51,true);
        $this->load->library('datatables');
        $this->datatables
...
$this->sma->checkPermissions(13,51);
    ...
    function profit_loss($start_date = NULL, $end_date = NULL)
    {
		...
		$this->sma->checkPermissions(13,44);
        if (!$start_date) {
    ...
    function register()
    {
       // $this->sma->checkPermissions('register');
		$this->sma->checkPermissions(13,35);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
    ...
    function getRrgisterlogs($pdf = NULL, $xls = NULL)
    {
		...
		$this->sma->checkPermissions(13,35,true);
        if ($this->input->get('user')) {
            $user = $this->input->get('user');
        } else {
            $user = NULL;
    ...
    {
        //$this->sma->checkPermissions();
		$this->sma->checkPermissions(13,48);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
...
$this->sma->checkPermissions(13,48,true);
        $reference_no = $this->input->get('reference_no') ? $this->input->get('reference_no') : NULL;
        $category = $this->input->get('category') ? $this->input->get('category') : NULL;
    ...
    {
        //$this->sma->checkPermissions();
		$this->sma->checkPermissions(13,45);
        if (!$this->Owner && !$this->Admin && $this->session->userdata('warehouse_id')) {
            $warehouse_id = $this->session->userdata('warehouse_id');
        }
        if (!$year) {
    ...
    {
       // $this->sma->checkPermissions();
		$this->sma->checkPermissions(13,46);
        if ($this->session->userdata('warehouse_id')) {
            $warehouse_id = $this->session->userdata('warehouse_id');
        }
        if (!$year) {
