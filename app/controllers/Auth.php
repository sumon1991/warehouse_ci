...
class Auth extends MY_Controller
{
    ...
    function users()
    {
       /* if ( ! $this->loggedIn) {
		...
		$this->sma->checkPermissions(17,9);
	...
	$this->sma->checkPermissions(17,9);
        ...
        $this->datatables
            //->select($this->db->dbprefix('users').".id as id, first_name, last_name, email, company, award_points, ".$this->db->dbprefix('groups').".name, ".$this->db->dbprefix('companies').".name, active")
			->select($this->db->dbprefix('users').".id as id, ".$this->db->dbprefix('users').".first_name, ".$this->db->dbprefix('users').".last_name, ".$this->db->dbprefix('users').".email, ".$this->db->dbprefix('groups').".name, ".$this->db->dbprefix('users').".company, ".$this->db->dbprefix('users').".award_points, CONCAT_WS(' ',c.first_name, c.last_name, '<br />', c.username) AS company_name, , ".$this->db->dbprefix('users').".active")
            ->from("users")
            ->join('groups', 'users.group_id=groups.id', 'left')
			->join('users c', 'users.created_by = c.id', 'left')
			->where($this->db->dbprefix('users').".id !=",1);
			if(hasAccess(17,24)) {
				$this->datatables->where($this->db->dbprefix('users').".created_by",$this->loginID);
			}
			...
			if(hasAccess(17,66))
			$actions .= "<a href='" . site_url('auth/view_tree/$1') . "' class='tip' title='" . lang("view_tree") . "'><i class=\"fa fa-sitemap\"></i></a> ";
			if(hasAccess(17,6))
			$actions .= "<a href='" . site_url('auth/user_permissions/$1') . "' class='tip' title='" . lang("permissions") . "'><i class=\"fa fa-tasks\"></i></a> ";
			if(hasAccess(17,7))
			$actions .= "<a href='" . site_url('auth/profile/$1') . "' class='tip' title='" . lang("edit_user") . "'><i class=\"fa fa-edit\"></i></a>";
			$actions .= "</div>";
    ...
    {
	$this->sma->checkPermissions(17,7);
    ...
    function profile($id = NULL)
    {
        ...
        $groups = $this->ion_auth->where("id >",1)->groups()->result_array();
...
$this->sma->checkPermissions(17,6);
...
$this->sma->checkPermissions(17,7);
    ...
    {
		$this->sma->checkPermissions(20,29);
        if ($this->input->post('id')) {
            $id = $this->input->post('id');
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
					$this->sma->checkPermissions(17,8);
...
if($this->input->post('form_action') == 'export_pdf') $this->sma->checkPermissions(17,20);
if($this->input->post('form_action') == 'export_excel') $this->sma->checkPermissions(17,21);
    ...
    {
$this->sma->checkPermissions(16,6,true);
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            redirect("auth/user_groups");
        }

        if ($this->form_validation->run() == TRUE && ($new_group_id = $this->auth_model->addGroup($data))) {
            $this->session->set_flashdata('message', lang('group_added'));
            redirect("auth/permissions/" . $new_group_id);
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'auth/create_group', $this->data);
        }
    }

    function edit_group($id)
    {
$this->sma->checkPermissions(16,7,true);
        if (!$id || empty($id)) {
            redirect('auth/user_groups');
        }

        $group = $this->auth_model->getGroupByID($id);

        $this->form_validation->set_rules('group_name', lang('group_name'), 'required|alpha_dash');

        if ($this->form_validation->run() === TRUE) {
            $data = array('name' => strtolower($this->input->post('group_name')), 'description' => $this->input->post('description'));
            $group_update = $this->auth_model->updateGroup($id, $data);
	...
	function view_tree($id = NULL)
    {
		$this->sma->checkPermissions(17,66);
	
		$this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');

		$this->data['id'] = $id;
		//$this->data['p'] = $this->auth_model->getGroupPermissions($id);
		$this->data['tree'] = $this->auth_model->getTree($id);
		//print '<pre>'; print_r($this->data['tree']); exit;
		$this->data['user'] = $this->auth_model->getUserByID($id);
		$g = $this->auth_model->getGroupByID($this->data['user']->group_id);
		//print '<pre>'; print_r($g); exit;
		$this->data['user']->group_name = '';
		if($g)
		$this->data['user']->group_name = $g->description;
		//print '<pre>'; print_r($this->data['user']); exit;
		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('auth/users'), 'page' => lang('users')), array('link' => '#', 'page' => lang('user_permissions')));
		$meta = array('page_title' => lang('view_tree'), 'bc' => $bc);
		$this->page_construct('auth/view_tree', $meta, $this->data);	
	}

    function user_permissions($id = NULL)
    {
		$this->sma->checkPermissions(17,26);
		if(!empty($_POST)) {
			$res = $this->auth_model->updatePermissions($id,"user",$this->input->post(NULL,TRUE));
			if($res) {
				$this->session->set_flashdata('message', lang("user_permissions_updated"));
				redirect(site_url("auth/updatePermissionsFiles/".$id."/user"));
			}
		}
		$this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
		...
		$this->data['id'] = $id;
		//$this->data['p'] = $this->auth_model->getGroupPermissions($id);
		$this->data['privileges'] = $this->auth_model->getPrivileges();
		//print '<pre>'; print_r($this->data['privileges']); exit;
		$this->data['user'] = $this->auth_model->getUserByID($id);
		//print '<pre>'; print_r($this->data['user']); exit;
		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('auth/user_permissions'), 'page' => lang('user_permissions')), array('link' => '#', 'page' => lang('user_permissions')));
		$meta = array('page_title' => lang('user_permissions'), 'bc' => $bc);
		$this->page_construct('auth/user_permissions', $meta, $this->data);	
	}
	
	
	
	function permissions($id = NULL)
    {$this->sma->checkPermissions(16,26);
		if(!empty($_POST)) {
			$res = $this->auth_model->updatePermissions($id, "group",$this->input->post(NULL,TRUE));
			if($res) {
				$this->session->set_flashdata('message', lang("group_permissions_updated"));
				redirect(site_url("auth/updatePermissionsFiles/".$id."/group"));
			}
		}
		$this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
		...
		$this->data['privileges'] = $this->auth_model->getPrivileges();
		//print '<pre>'; print_r($this->data['privileges']); exit;
		$this->data['group'] = $this->auth_model->getGroupByID($id);

		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('auth/user_groups'), 'page' => lang('groups_permissions')), array('link' => '#', 'page' => lang('group_permissions')));
		$meta = array('page_title' => lang('group_permissions'), 'bc' => $bc);
		$this->page_construct('auth/permissions', $meta, $this->data);

    }
	public function updatePermissionsFiles($id,$category)
	{
		$data['id'] = $id;
		$data['category'] = $category;
		
		$this->load->view($this->Settings->theme."/views/auth/update_permissions_files",$data);
	}
	public function do_updatePermissionsFiles($id = "",$category = "")
	{
		$id = $this->input->post("id");
		$category = $this->input->post("category");
		//print_r($_POST);exit;
		$render = "<?php  defined('BASEPATH') OR exit('No direct script access allowed');";
		$find = array("'","\"");
		$replace = array("","");
		
		$d = $this->site->getFilePermissions($category);
		//print '<pre>'; print_r($d);exit;
		foreach($d as $v) {
$render .= '
config["'.$v['category'].'"]['.$v['record'].']['.$v['privilege_module_id'].']['.$v['rule_id'].'] = '.$v['status'].';';
		}
		
		$find = array("config[");
		$replace = array('$config[');
		if($category == "user")
		$path = "./app/config/permissions_users.php";
		else
		$path = "./app/config/permissions_groups.php";
		
		$render = str_replace($find,$replace,$render);
		$this->site->write_file($path,$render);
	
		$return['result'] = 1;
		$return['message'] = lang("system_is_redirecting");
		if($category == "user")
		$return['redirect_link'] = site_url("auth/user_permissions/".$id);
		else
		$return['redirect_link'] = site_url("auth/permissions/".$id);
		//$this->session->set_userdata("success_message","Routes are published successfully..");
		//redirect(site_url("auth/permissions/".$id));
		die(json_encode($return));
	}

    function user_groups()
    {
$this->sma->checkPermissions(16,9);
       /* if (!$this->Owner) {
        ...
        $this->page_construct('auth/user_groups', $meta, $this->data);
    }

    function delete_group($id = NULL)
    {
		$this->sma->checkPermissions(16,8);
      /*  if (!$this->Owner) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect('welcome', 'refresh');
        }*/
        if ($this->auth_model->checkGroupUsers($id)) {
            $this->session->set_flashdata('error', lang("group_x_b_deleted"));
            redirect("auth/user_groups");
        }
        if ($this->auth_model->deleteGroup($id)) {
