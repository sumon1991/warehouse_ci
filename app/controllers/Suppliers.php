...
class Suppliers extends MY_Controller
{
    ...
    {
        $this->sma->checkPermissions(12,9);
...
$this->sma->checkPermissions(12,9);
        ...
        $this->datatables
			...
			if(hasAccess(12,7))
			$actions .= "<a class=\"tip\" title='" . $this->lang->line("edit_supplier") . "' href='" . site_url('suppliers/edit/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> ";
			if(hasAccess(17,0)) {
				if(hasAccess(17,9))
				$actions .= "<a class=\"tip\" title='" . $this->lang->line("list_users") . "' href='" . site_url('suppliers/users/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-users\"></i></a> ";
				if(hasAccess(17,6))
				$actions .= "<a class=\"tip\" title='" . $this->lang->line("add_user") . "' href='" . site_url('suppliers/add_user/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-plus-circle\"></i></a> ";
			}
			if(hasAccess(12,8))
			$actions .= "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_supplier") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('suppliers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
			$actions .= "</div>";
    ...
    function view($id = NULL)
    {
	   ...
	   $this->sma->checkPermissions(12,9,true);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
...
$this->sma->checkPermissions(12,6,true);
    ...
    {
      //  $this->sma->checkPermissions(false, true);
$this->sma->checkPermissions(12,7,true);
    ...
    {
       // $this->sma->checkPermissions(false, true);
$this->sma->checkPermissions(17,9,true);
        if ($this->input->get('id')) {
...
$this->sma->checkPermissions(17,6);
    ...
    function import_csv()
    {
		...
		$this->sma->checkPermissions(12,23);
...
$this->sma->checkPermissions(12,8,true);
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
					$this->sma->checkPermissions(12,8);
...
if($this->input->post('form_action') == 'export_pdf') $this->sma->checkPermissions(12,20);
if($this->input->post('form_action') == 'export_excel') $this->sma->checkPermissions(12,21);
