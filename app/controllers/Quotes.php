...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
       // $this->sma->checkPermissions('index');
 	   $this->sma->checkPermissions(8,9,true);
        if (!$warehouse_id) {
 ...
 $this->sma->checkPermissions(8,11);
        if ($this->input->get('id')) {
            $quote_id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
 ...
 $this->sma->checkPermissions(8,22);
 ...
 $this->sma->checkPermissions(8,10,true);
        if ($this->input->get('id')) {
            $quote_id = $this->input->get('id');
        }
        $inv = $this->quotes_model->getQuoteByID($quote_id);
 ...
 $this->sma->checkPermissions(8,8,true);
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
...
if($this->input->post('form_action') == 'export_pdf') $this->sma->checkPermissions(8,20);
if($this->input->post('form_action') == 'export_excel') $this->sma->checkPermissions(8,21);
