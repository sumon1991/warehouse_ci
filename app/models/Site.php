...
class Site extends CI_Model
{
    ...
    {
        $q = $this->db->get_where('privileges_modules', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
        $q = $this->db->get_where('privileges_rules', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function insertUpdateRulesModules($id,$arr)
	{
		$old_ids = array();
		$new_ids = array();
		$deleted_ids = array();
		$this->db->select('*');
		$this->db->from("privileges_rules_access");
		$cond = array("rule_id"=>$id);
		$this->db->where($cond);
		$query = $this->db->get();
		$results = $query->result_array();
		if(!empty($results)) {
			foreach($results as $res) {
				array_push($old_ids,$res['privilege_module_id']);
			}
		}
		foreach($arr as $ar) {
		...
		if(!empty($deleted_ids)) {
			$q = 'DELETE FROM sma_privileges_rules_access WHERE rule_id = '.$id.' AND privilege_module_id IN ('.implode(',',$deleted_ids).')';
			$this->db->query($q);
		}
		if(!empty($new_ids)) {
			foreach($new_ids as $new_id) {
				$data['privilege_module_id'] = $new_id;
				$data['rule_id'] = $id;
				$this->db->insert("privileges_rules_access",$data);
			}
		}
	}
	public function getRuleSelectedModules($id)
	{
		$this->db->select('*');
		$this->db->from("privileges_rules_access");
		$cond = array(
		...
		$results = array();
		foreach($arr as $ar) {
			array_push($results,$ar['privilege_module_id']);
		}
		return $results;
	}
	public function insertUpdateModulesRules($id,$arr)
	{
		$old_ids = array();
		$new_ids = array();
		$deleted_ids = array();
		$this->db->select('*');
		$this->db->from("privileges_rules_access");
		$cond = array("privilege_module_id"=>$id);
		$this->db->where($cond);
		$query = $this->db->get();
		$results = $query->result_array();
		if(!empty($results)) {
			foreach($results as $res) {
				array_push($old_ids,$res['rule_id']);
			}
		}
		foreach($arr as $ar) {
			if(!in_array($ar,$old_ids)) {
		...
		if(!empty($deleted_ids)) {
			$q = 'DELETE FROM sma_privileges_rules_access WHERE privilege_module_id = '.$id.' AND rule_id IN ('.implode(',',$deleted_ids).')';
			$this->db->query($q);
		}
		if(!empty($new_ids)) {
			foreach($new_ids as $new_id) {
				$data['rule_id'] = $new_id;
				$data['privilege_module_id'] = $id;
				$this->db->insert("privileges_rules_access",$data);
			}
		}
	}
	public function getModuleSelectedRules($id)
	{
		$this->db->select('*');
		$this->db->from("privileges_rules_access");
		$cond = array(
			'privilege_module_id'=>$id,
		);
		$this->db->where($cond);
		$query = $this->db->get();
		$arr = $query->result_array();
		$results = array();
		foreach($arr as $ar) {
			array_push($results,$ar['rule_id']);
		}
		return $results;
	}
	
	public function getFilePermissions($category)
	{
		$this->db->select("*");
		$this->db->from("privileges_given");
		$this->db->where("category",$category);
		$query = $this->db->get();
		//echo $category.$this->db->last_query();exit;
		$results = $query->result_array();
    ...
    {
        $q = $this->db->get_where('translations', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
