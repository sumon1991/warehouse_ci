...
class Auth_model extends CI_Model
{
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE)
    {
		...
		$user_data['created_by'] = $this->loginID;
    ...
    public function deleteGroup($id)
    {
        if ($this->db->delete('groups', array('id' => $id))) {
			$this->db->delete('privileges_given', array('record' => $id,"category"=>"group"));
            return true;
        }
        return FALSE;
    }
    ...
    {
        $q = $this->db->get_where('permissions_users', array('user_id' => $id),1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
		else {
			$User = $this->getUserByID($id);
			//print $User->group_id.'<pre>'; print_r($User);exit;
			$q1 = $this->db->get_where('permissions', array('group_id' => $User->group_id),1);
        	if ($q1->num_rows() > 0) {
           		return $q1->row();
        	}
			else {
				$fields = $this->db->list_fields('permissions_users');
				$i = array();
				$i['user_id'] = $id;
				...
				$this->db->insert("permissions_users",$i);
				$q = $this->db->get_where('permissions_users', array('user_id' => $id),1);
				if ($q->num_rows() > 0) {
					return $q->row();
				}
    ...
    {
		//echo $id;exit;
        if ($this->db->update('permissions_users', $data, array('user_id' => $id)) && $this->db->update('users', array('show_price' => $data['products-price'], 'show_cost' => $data['products-cost']), array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function getGroupPermissions($id)
    {
        ...
        if ($q->num_rows() > 0) {
			...
			$fields = $this->db->list_fields('permissions');
				...
				$i['group_id'] = $id;
				foreach($fields as $field) {
					if($field != "group_id" && $field != "id") {
						$i[$field] = 0;
					}
				}
				$this->db->insert("permissions",$i);
				$q = $this->db->get_where('permissions', array('group_id' => $id),1);
				if ($q->num_rows() > 0) {
					return $q->row();
				}
	...
	{
		$query = $this->db->get("privileges_modules");
		$results = $query->result_array();
		if(!empty($results)) {
			foreach($results as $key => $val) {
				$this->db->select($this->db->dbprefix('privileges_rules').".id, ".$this->db->dbprefix('privileges_rules').".name");
				$this->db->from("privileges_rules");
				$this->db->join("privileges_rules_access","privileges_rules_access.rule_id = privileges_rules.id");
				$this->db->where("privileges_rules_access.privilege_module_id",$val['id']);
				$this->db->group_by("privileges_rules.id");
				$query = $this->db->get();
				$results[$key]['rules'] = $query->result_array();
			}
		}
		return $results;
	}
	
 	public function updatePermissions($id, $category, $data = array())
    {
		if(empty($data)) exit("error");
		$this->db->delete("privileges_given",array("category"=>$category,"record"=>$id));
		$privileges = $this->auth_model->getPrivileges();
		$rules = $data['rule'];
		$hide = $data['hide'];
		foreach($privileges as $priv) {
			$status = 0;
			if(isset($hide[$priv['id']]) && $hide[$priv['id']] == 1 ) {
				$status = 1;
			}
			$this->db->insert("privileges_given",array(
				"category"=>$category,
				"record"=>$id,
				"privilege_module_id"=>$priv['id'],
				"rule_id"=>enableDisableID(),
				"status"=>$status
			));
			foreach($priv['rules'] as $rule) {
				$status = 0;
				if(isset($rules[$priv['id']][$rule['id']]) && $rules[$priv['id']][$rule['id']] == 1 ) {
					$status = 1;
				}
				$this->db->insert("privileges_given",array(
					"category"=>$category,
	...
	function getTree($userID) {
		$users1 = $this->getUsersByParentID($userID);
		$children = array();
		if(!empty($users1)) {
			foreach ($users1 as $k => $user1) {
				$children[$k] = $user1;
				$children[$k]['sub'] = $this->getTree($user1['id']);
			}
		}
		return $children;
	}

	function getUsersByParentID($id)
	{
		$q = "SELECT sma_users.id, sma_users.username, sma_users.email, sma_users.first_name, sma_users.last_name, sma_users.company, sma_users.phone, sma_users.created_by, sma_groups.description AS group_name FROM sma_users JOIN sma_groups ON sma_groups.id = sma_users.group_id WHERE sma_users.created_by = ".$id;
		//echo $q.'<br />============<br />';
